...
# 制御バリア関数
def apply_control_barrier_function(a, x, k=4.5):
    """
    制御バリア関数 u = (a*0.01 + k * (x - h)) * 100
    - a: コントローラー入力値
    - x: 距離センサー値（m）
    - k: バリア関数のゲイン
    - h: バリア距離（m）
    """
    if x < 0:  # 距離センサーが無効値の場合はコントローラー値をそのまま使用
        return a
    elif x <= h and x >=0.04 :  # baria min distance rparame-ta
        return (a * 0.01 + k * (x - h)) * 100
    else:  # x >= h
        return a
...
def control_motors():
    ...
    while True:
        left_speed = apply_control_barrier_function(joy_values["joy_ly"], distance_value)
        right_speed = apply_control_barrier_function(joy_values["joy_ry"], distance_value)
